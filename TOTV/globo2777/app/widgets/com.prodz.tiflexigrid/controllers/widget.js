...
var init = function(opts){
	...
	onItemLongClick = params.onItemLongClick || function(){Ti.API.info('TiFlexiGrid -> onItemClick is not defined.');};
...
var addGridItems = function(args){
	...
	for (var x=0;x < data.length; x++){
		addGridItem(data[x], x);
	}
};

var addGridItem = function(item, x){
	var frame = Ti.UI.createView({
		width:columnWidth,
		height:columnWidth + itemsOptions.heightDelta,
		backgroundColor:itemsOptions.backgroundColor[x],
		top:0,
		left:0,
		right:0,
		bottom:0,
		borderColor:itemsOptions.backgroundColor[x],
	...
	if(!OS_IOS){
		overlay.addEventListener('longclick', function(e){
			onItemLongClick(e);
		});
	}
	else{
...
var setonItemLongClick = function(fnt){
	onItemLongClick = fnt || function(){Ti.API.info('TiFlexiGrid -> onItemLongClick is not defined.');};
};
...
exports.setonItemLongClick = setonItemLongClick;